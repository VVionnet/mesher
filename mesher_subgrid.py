...
def rasterize_elem(raster, feature, key, aggMethod):
    ...
    elif aggMethod == 'frac_neg':
        if masked.count() > 0:
            frac_nan_tri = 1.-masked.count()/masked_tri.count()
            if(frac_nan_tri < 0.5):
               output = float(np.sum(masked<0.))/float(masked.count())
    elif aggMethod == 'mean_neg':
        masked_sel = masked[masked<0.]
        if  masked.count() > 0 and masked_sel.count() > 0:
            frac_nan_tri = 1.-masked.count()/masked_tri.count()
            if(frac_nan_tri < 0.5):
               output = float(masked_sel.mean())
    elif aggMethod == 'std_neg':
        masked_sel = masked[masked<0.]
        if  masked.count() > 0 and masked_sel.count() > 0:
            frac_nan_tri = 1.-masked.count()/masked_tri.count()
            if(frac_nan_tri < 0.5):
